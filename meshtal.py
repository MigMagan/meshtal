...
class MeshTally:
    ...
    def RoNormalization(self, ptrac_file, outp_file, method="pointsample", pformat="Ascii", saveRomesh=None, error_threshold=0.2):
        import ptrac
        Ro,Ro_error=ptrac.romesh(self, ptrac_file, outp_file, method, pformat, None)
        ...
        for i, j, k, e in np.ndindex(Result.iints, Result.jints, Result.kints, Result.eints+1):
            if Ro[i,j,k] != 0 and Ro_error[i, j, k] <= error_threshold:
                if Ro[i,j,k] > 0:
        ...
        if saveRomesh != None:
            ...
            Romesh.part = 'Density'
            Romesh.probID = "Generated by meshtal class from {probID}".format(probID=self.probID)
            ...
            for (i, j, k) in np.ndindex(Romesh.iints, Romesh.jints, Romesh.kints):
                if Ro[i,j,k] != 0 and Ro_error[i, j, k] <= error_threshold:
                    if Ro[i,j,k] > 0:
                        ...
                        Romesh.error[i,j,k,:] = Ro_error[i, j, k]
                else:
