...
class MeshTally:
    ...
    def interpolate(self, XYZ):
        if self.geom == 'Cyl':
            print('XYZ coordinates:', '{0:.2f} {1:.2f} {2:.2f} '.format(XYZ[0],XYZ[1],XYZ[2]))
            XYZ = conv_Cilind(self.axis, self.vec, self.origin, XYZ)
            print('CYL coordinates:', '{0:.2f} {1:.2f} {2:.2f} '.format(XYZ[0],XYZ[1],XYZ[2]))
        if len(XYZ) != 3:
...
def ratio(RefTal,DivTal):
    ...
    def RoNormal(meshtal, ptrac_file, outp_file, method="pointsample", pformat="Ascii",getRo=True):
        import ptrac
        Ro,Ss=ptrac.romesh(meshtal, ptrac_file, outp_file, method, pformat, None)
        Result = copy (meshtal, exclude=['probID', 'value', 'error'])
        Result.probID = "Generated by meshtal class from {probID}".format(probID=meshtal.probID)
        Result.comment+= " Normalized by density from {0}".format(ptrac_file)
        if getRo == True:
            Romesh = MeshTally(meshtal.iints, meshtal.jints, meshtal.kints, 1, meshtal.geom)
            Romesh.part = 'Density'
            Romesh.probID = "Generated by meshtal class from {probID}".format(probID=meshtal.probID)
            Romesh.comment+= " Density from {0}".format(ptrac_file)
            for key in vars(meshtal).keys():
                if key not in ['part','probID','comment','iints','jints','kints','eints','ebins','geom']:
                    vars(Romesh)[key] = vars(meshtal)[key]
        for (i, j, k, e) in np.ndindex(Result.iints, Result.jints, Result.kints, Result.eints+1):
            if Ro[i,j,k] != 0:
                if Ro[i,j,k] > 0:
                    Result.value[i,j,k,e] = meshtal.value[i,j,k,e]/Ro[i,j,k]
                    if getRo == True:
                        Romesh.value[i,j,k,:] = Ro[i,j,k]
                else:
                    Result.value[i,j,k,e] = meshtal.value[i,j,k,e]/Ro[i,j,k]*-1
                    if getRo == True:
                        Romesh.value[i,j,k,:] = Ro[i,j,k]*-1
                Result.error[i,j,k,e] = meshtal.error[i,j,k,e]
                if getRo == True:
